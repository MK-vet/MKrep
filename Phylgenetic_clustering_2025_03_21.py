...
"""
Complete Script for Phylogenetic Clustering and Binary Trait Analysis
========================================================================

Author: MK-vet (with modifications)
Date: 2025-03-18

Features:
- Phylogenetic clustering with tree-aware methods (TreeCluster, Phydelity-inspired) that respect tree structure,
- Binary trait analysis (AMR, virulence, etc.),
- Evolutionary metrics (PD, pairwise distance),
- Interactive HTML report (DataTables, Plotly),
- Automatic rounding to two decimals (except 'Strain_ID'),
- Color-coded dendrogram-style phylogenetic tree with clusters,
- Shared/unique features (improved implementations),
- Multiple testing correction (FDR) for pairwise comparisons,
- Automatic scanning for CSV/image output files,
- Reproducible results via fixed random seeds and controlled threading,
- Integrated automatic method for selecting the optimal number of clusters using silhouette analysis.
"""
...
class PhylogeneticCore:
    ...
    def tree_to_distance_matrix(tree, parallel=False, n_jobs=None):
        ...
        if parallel:
            distance_matrix = ParallelProcessor.parallel_tree_distance_matrix(tree, terminals, n_jobs=n_jobs)
        else:
            n = len(terminals)
            distance_matrix = np.zeros((n, n))
...
class TreeAwareClusteringModule:
    """
    Performs clustering that respects the phylogenetic tree structure.
    Implements a TreeCluster algorithm with monophyletic constraints and a Phydelity-inspired method.
    """
    def __init__(self, tree, terminals, n_clusters_range=(2, 20), seed=42):
        self.tree = tree
    ...
    def tree_cluster_algorithm(self, distance_matrix, method="max", threshold=None):
        ...
        internal_nodes = [clade for clade in self.tree.find_clades(order='postorder') if not clade.is_terminal()]
        ...
        for node in internal_nodes:
            ...
            if method == "max":
                is_valid = self._check_max_constraint(terminals_in_subtree, distance_matrix, threshold)
            elif method == "sum":
                is_valid = self._check_sum_constraint(terminals_in_subtree, distance_matrix, threshold)
            else:
                is_valid = self._check_avg_constraint(terminals_in_subtree, distance_matrix, threshold)
    ...
    def phydelity_clustering(self, distance_matrix):
        ...
        print(f"Phydelity identified {len(np.unique(cluster_labels))} clusters with silhouette {silhouette:.2f}")
...
class ClusteringModule:
            ...
            for _ in range(self.n_ensemble):
                for n_clusters in range(self.n_clusters_range[0], self.n_clusters_range[1] + 1):
                    methods = [
                        ...
                        (DBSCAN, {'eps': best_params.get('eps', 0.5), 'min_samples': best_params.get('min_samples', 5)}),
    ...
    def assign_outliers_to_clusters(self, embeddings, mask, labels):
        ...
        cluster_centers = np.array([
            embeddings[mask][labels == lbl].mean(axis=0)
...
class EvolutionaryAnalysis:
    ...
    def evolutionary_cluster_analysis(tree, labels, strain_names, mask):
        ...
        for cluster_id in np.unique(labels):
            ...
            if len(cluster_strains) <= 1:
                cluster_info.append((cluster_id, cluster_strains.tolist(), 0.0, 0.0, 0, len(cluster_strains)))
            else:
                cluster_info.append(EvolutionaryAnalysis._analyze_cluster_evolution(tree, cluster_id, cluster_strains))
        df_clusters = pd.DataFrame(cluster_info, columns=["Cluster_ID", "Strains", "PD", "MeanPairwiseDist", "InternalNodes", "SubtreeTerminals"])
        return df_clusters
    ...
    def _analyze_cluster_evolution(tree, cluster_id, cluster_strains):
        try:
            ...
            distances = [tree.distance(s1, s2) for i, s1 in enumerate(cluster_strains) for s2 in cluster_strains[i+1:]]
            ...
            return (cluster_id, cluster_strains.tolist(), pd_value, mean_pairwise_dist, internal_nodes, len(mrca.get_terminals()))
        except:
...
class DataLoader:
    ...
    def load_and_merge_data(self, clusters_path):
        try:
            ...
            amr_genes_df = pd.read_csv(amr_path)
            virulence_genes_df = pd.read_csv(vir_path)

            for df in [clusters_df, mic_df, amr_genes_df, virulence_genes_df]:
...
class Visualizer:
    ...
    def plot_cluster_distribution(self, merged_df):
        cluster_stats = merged_df.groupby('Cluster').agg({
            'Strain_ID': ['count', lambda x: len(x) / len(merged_df) * 100]
        }).round(2)
        ...
        for i, p in enumerate(ax.patches):
            ax.annotate(f"{int(cluster_stats['Strain_Count'].iloc[i])}",
    ...
    def plot_phylogenetic_tree(self, tree, labels, strain_names, mask, output_file="phylogenetic_tree.png"):
        """
        Updated to ensure color-coded strain labels and branches in the phylogenetic tree.
        """
        ...
        # Create legend
        legend_elements = [Patch(facecolor=colors[i], label=f'Cluster {lbl}') for i, lbl in enumerate(unique_labels)]
...
class TraitAnalyzer:
    ...
    def _analyze_trait_frequencies(self, df, suffix=''):
        ...
        trait_freq.to_csv(os.path.join(self.output_folder, f"trait_frequencies_{suffix.lower()}.csv"))
        ...
        plt.xlabel("Trait")
        plt.tight_layout()
        plt.savefig(os.path.join(self.output_folder, f"trait_frequencies_heatmap_{suffix.lower()}.png"))
        plt.close()
    ...
    def _perform_statistical_tests(self, df, suffix='', alpha=0.05):
        ...
        _, results_df['p_adjusted'], _, _ = multipletests(results_df['p_value'], alpha=alpha, method='fdr_bh')
        results_df['significant'] = results_df['p_adjusted'] < alpha
        results_df.to_csv(os.path.join(self.output_folder, f"statistical_tests_{suffix.lower()}.csv"), index=False)
    ...
    def _calculate_feature_importance(self, df, suffix='', n_bootstrap=1000):
        ...
        importance_df.to_csv(os.path.join(self.output_folder, f"feature_importance_{suffix.lower()}.csv"), index=False)
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(self.output_folder, f"feature_importance_{suffix.lower()}.png"), dpi=300)
        plt.close()
    ...
    def bootstrap_feature_importance(self, df, n_bootstrap=1000):
        ...
        df_scores.to_csv(os.path.join(self.output_folder, "bootstrap_feature_importance.csv"), index=False)
        return df_scores
    ...
    def log_odds_ratio_analysis(self, df):
        ...
        for trait in traits:
            ...
            for cl in sorted(df['Cluster'].unique()):
                ...
                cluster_data.append({"Cluster": cl, "Feature": trait, "Log_Odds_Ratio": round(log_odds, 2)})
    ...
    def association_rule_mining(self, df, min_support=0.05, min_confidence=0.7, max_features=100):
        ...
        try:
            ...
            while len(selected_features) > 10:
                try:
                    print(f"Attempting apriori with {len(selected_features)} features, min_support={adaptive_min_support}")
                    frequent_itemsets = apriori(trait_df, min_support=adaptive_min_support,
                    ...
                    print(f"MemoryError - reducing to {len(selected_features)} features, min_support={adaptive_min_support}")
    ...
    def label_shared_unique_features(self, df, presence_threshold=0.3):
        ...
        try:
            if 'Cluster' not in df.columns:
                ...
                return (
                    pd.DataFrame(columns=["Feature", "Clusters", "NumClusters", "Count", "Percent_in_Clusters"]),
            ...
            if not df_shared.empty:
                df_shared = df_shared.sort_values(by=["NumClusters", "Count"], ascending=[False, False])
    ...
    def pairwise_fdr_post_hoc(self, df, alpha=0.05, method='fdr_bh'):
        ...
        try:
            binary_cols = [col for col in df.columns if col not in ['Strain_ID', 'Cluster']]
            clusters = sorted(df['Cluster'].unique())
            print(f"Performing pairwise post-hoc analysis with {len(clusters)} clusters and {len(binary_cols)} features")
            if len(clusters) < 2:
                print("Not enough clusters for pairwise analysis.")
                df_posthoc = pd.DataFrame(columns=["Feature", "ClusterA", "ClusterB", "p_value_raw", "p_value_adj", "significant"])
                df_posthoc.to_csv(os.path.join(self.output_folder, "pairwise_fdr_post_hoc.csv"), index=False)
                return df_posthoc
            ...
            if not df_posthoc.empty:
                df_posthoc = df_posthoc.sort_values(by=["p_value_adj", "Feature", "ClusterA", "ClusterB"])
                df_posthoc = df_posthoc.round(4)
                print(f"Found {sum(df_posthoc['significant'])} significant relationships after FDR correction")
            else:
    ...
    def bootstrap_log_odds(self, df, n_bootstrap=100):
        ...
        try:
            ...
            traceback.print_exc()
            df_boot = pd.DataFrame(columns=[
                "Feature", "Bootstrap_Log_Odds_Mean", "Bootstrap_Log_Odds_Std",
                "Bootstrap_Log_Odds_Lower_CI", "Bootstrap_Log_Odds_Upper_CI", "Significant"
            ])
            df_boot.to_csv(os.path.join(self.output_folder, "bootstrap_log_odds.csv"), index=False)
            return df_boot
...
class MCAAnalyzer:
    ...
    def perform_mca_analysis(self, merged_df):
        try:
            ...
            row_coords.to_csv(os.path.join(self.output_folder, "mca_row_coordinates.csv"), index=False)
            col_coords.to_csv(os.path.join(self.output_folder, "mca_column_coordinates.csv"), index=False)
            ...
            summary_df = pd.DataFrame({
                ...
                "Cumulative_Explained_Inertia": [round(sum(explained_inertia[:i+1]), 4) for i in range(len(explained_inertia))]
            })
            ...
            mca_guide.to_csv(os.path.join(self.output_folder, "mca_interpretation_guide.csv"), index=False)
...
class HTMLReportGenerator:
    ...
    def _create_interactive_tree_plot(self, tree_path):
        ...
        try:
            ...
            Phylo.draw(
                tree, axes=ax, do_show=False,
                branch_labels=None,
                label_func=lambda x: x.name if (x.is_terminal() and x.name in cluster_map) else None
            )
        ...
        for file in all_files:
            ...
            if os.path.isfile(file_path):
                ...
                if size_kb < 5000 and file.lower().endswith(('.csv', '.txt')):
                    ...
                    download_link = "<span class='btn btn-sm btn-secondary disabled'>No direct download</span>"
    ...
    def generate_report(self, analysis_results, config, output_file="phylogenetic_report.html"):
        ...
        if 'summary_stats' in analysis_results and isinstance(analysis_results['summary_stats'], pd.DataFrame):
        ...
        if os.path.exists(dist_csv):
            ...
            fig_dist = px.bar(
                dist_df, x='Cluster', y='Percentage',
                title='Cluster Distribution (%) - Interactive',
                labels={'Cluster': 'Cluster', 'Percentage': '% of Strains'},
                text='Strain_Count'
            )
            fig_dist.update_traces(textposition='outside')
            results['cluster_distribution'] = fig_dist.to_html(full_html=False, include_plotlyjs=False)
        else:
        ...
        if os.path.exists(evo_rates_csv):
            ...
            fig_rates = px.bar(
                df_rates, x='Cluster_ID', y='EvolutionRate',
        ...
        if 'best_silhouette' in analysis_results:
            df_val = pd.DataFrame({"SilhouetteScore": [round(analysis_results['best_silhouette'], 2)]})
        ...
        if os.path.exists(mca_col_csv) and os.path.exists(mca_summary_csv):
            ...
            if os.path.exists(mca_guide_path):
                ...
                guide_html = self._df_to_interactive_table(
                    guide_df,
                    table_id="table-mca-guide",
                    filename="mca_interpretation_guide"
                )
            else:
                guide_html = ""
            results['mca_column_plot'] = f"""
...
class Config:
    ...
    def __init__(self, **kwargs):
        ...
        self.output_folder = kwargs.get('output_folder', 'phylogenetic_clustering_results_advanced17xY')
        ...
        self.virulence_genes_file = kwargs.get('virulence_genes_file', 'Virulence3.csv')
...
class PhylogeneticAnalysis:
    """Main pipeline orchestrating the analysis."""
    ...
    def determine_adaptive_cluster_range(self, embeddings, distance_matrix=None):
        ...
        print(f"Adaptively determined cluster range: {min_k}-{max_k} (based on {n_samples} samples, {density_peaks} density peaks)")
        return (min_k, max_k)
    ...
    def test_multiple_clustering_methods(self, tree_clustering, distance_matrix):
        ...
        for method in ["max", "sum", "avg"]:
            ...
            for threshold_factor in [0.8, 1.0, 1.2, 1.5]:
                if method == "max":
                    threshold = tree_clustering._auto_threshold_max(conservative_factor=5.0) * threshold_factor
                elif method == "sum":
                    threshold = tree_clustering._auto_threshold_sum(conservative_factor=5.0) * threshold_factor
                else:
                    threshold = tree_clustering._auto_threshold_avg(conservative_factor=5.0) * threshold_factor

                labels = tree_clustering.tree_cluster_algorithm(distance_matrix, method=method, threshold=threshold)
                if len(np.unique(labels)) > 1:
                    try:
                        sil = silhouette_score(distance_matrix, labels, metric='precomputed')
                        print(f"Method: {method}, Threshold: {threshold:.4f}, Silhouette: {sil:.4f}, Clusters: {len(np.unique(labels))}")
                        if sil > best_silhouette:
                            best_silhouette = sil
                            best_labels = labels
                            best_method = f"TreeCluster-{method}"
                            best_params = {"threshold": threshold, "threshold_factor": threshold_factor}
                    except Exception as e:
                        print(f"Error: {e}")
        ...
        optimal_n, optimal_labels, optimal_score = self.determine_optimal_clusters(
            embeddings,
            cluster_range=self.adaptive_cluster_range  # Using the adaptive range
        )
