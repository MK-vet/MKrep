...
def generate_random_tree_newick(
    ...
    Returns:
        Tuple of (newick_string, cluster_assignments_dict)
    """
...
def generate_clustered_tree_newick(
    ...
    branch_length_mean: float = 0.05,
    random_state: Optional[int] = 42,
) -> Tuple[str, List[int], Dict[int, List[str]]]:
    """
    ...
    Returns:
        Tuple of (newick_string, cluster_labels, cluster_members)
    """
    rng = np.random.default_rng(random_state)
    ...
    return tree_str, final_labels, cluster_members
...
def save_synthetic_phylo_data(
    ...
    amr_df: pd.DataFrame,
    vir_df: pd.DataFrame,
    metadata: SyntheticPhyloMetadata,
...
def validate_synthetic_phylo_data(
    ...
    if tree_newick.endswith(";") and "(" in tree_newick:
        results["checks"].append("✓ Tree is valid Newick format")
    else:
