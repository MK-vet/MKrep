"""
Unit tests for strepsuis_amrpat core analysis functions.
...
class TestAnalyzerFileValidation:
    ...
    def test_run_with_missing_mic_file(self):
        """Test that run() raises error if MIC.csv is missing."""
        with tempfile.TemporaryDirectory() as tmpdir:
            config = Config(data_dir=tmpdir)
            analyzer = MDRAnalyzer(config=config)

            # Create AMR_genes.csv but not MIC.csv
            pd.DataFrame({"Strain": ["S1"], "Gene": ["tet"]}).to_csv(
                os.path.join(tmpdir, "AMR_genes.csv"), index=False
            )

            with pytest.raises(FileNotFoundError, match="MIC.csv"):
                analyzer.run()

    def test_run_with_missing_amr_genes_file(self):
        """Test that run() raises error if AMR_genes.csv is missing."""
        with tempfile.TemporaryDirectory() as tmpdir:
            config = Config(data_dir=tmpdir)
            analyzer = MDRAnalyzer(config=config)

            # Create MIC.csv but not AMR_genes.csv
            pd.DataFrame({"Strain": ["S1"], "Antibiotic": ["PEN"], "MIC": [0.5]}).to_csv(
                os.path.join(tmpdir, "MIC.csv"), index=False
            )

            with pytest.raises(FileNotFoundError, match="AMR_genes.csv"):
                analyzer.run()

    def test_run_with_both_files_missing(self):
        """Test that run() raises error listing both missing files."""
        with tempfile.TemporaryDirectory() as tmpdir:
            config = Config(data_dir=tmpdir)
            analyzer = MDRAnalyzer(config=config)

            with pytest.raises(
                FileNotFoundError, match="MIC.csv.*AMR_genes.csv|AMR_genes.csv.*MIC.csv"
            ):
                analyzer.run()
