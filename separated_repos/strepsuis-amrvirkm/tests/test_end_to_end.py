...
def test_reproducibility(mini_dataset):
    ...
    # Run analysis twice
    analyzer1 = ClusterAnalyzer(config=config)
    analyzer2 = ClusterAnalyzer(config=config)
...
def test_configuration_impact(mini_dataset):
    ...
    analyzer_low = ClusterAnalyzer(config=config_low)
    analyzer_high = ClusterAnalyzer(config=config_high)
