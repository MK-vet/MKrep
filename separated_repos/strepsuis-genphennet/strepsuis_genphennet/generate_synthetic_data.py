...
def validate_synthetic_network_data(
    ...
    if all_binary:
        results["checks"].append("✓ Data is binary")
    else:
    ...
    if detected_associations > 2:
        results["checks"].append(f"✓ True associations detectable: {detected_associations}/5")
    else:
        results["warnings"].append(
            f"⚠ Only {detected_associations}/5 associations clearly detectable"
        )
