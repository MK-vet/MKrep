...
def test_reproducibility(mini_dataset):
    ...
    # Run analysis twice
    analyzer1 = GenPhenAnalyzer(config=config)
    analyzer2 = GenPhenAnalyzer(config=config)
...
def test_configuration_impact(mini_dataset):
    ...
    analyzer_low = GenPhenAnalyzer(config=config_low)
    analyzer_high = GenPhenAnalyzer(config=config_high)
