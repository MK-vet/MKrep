"""
Comprehensive workflow tests for strepsuis-genphen.
...
def test_complete_analysis_workflow(workflow_data):
    ...
    with patch('strepsuis_genphen.mdr_analysis_core.setup_environment') as mock_setup:
...
def test_reproducibility_workflow(workflow_data):
    ...
    analyzer1 = GenPhenAnalyzer(
        data_dir=str(workflow_data["data_dir"]),
        output_dir=str(workflow_data["output_dir"]),
        bootstrap_iterations=100
    )
    
    analyzer2 = GenPhenAnalyzer(
        data_dir=str(workflow_data["data_dir"]),
        output_dir=str(workflow_data["output_dir"]),
        bootstrap_iterations=100
    )
