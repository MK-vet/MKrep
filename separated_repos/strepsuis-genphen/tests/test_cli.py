...
def test_cli_help(capsys, monkeypatch):
    ...
    monkeypatch.setattr(sys, "argv", ["strepsuis-genphen", "--help"])
...
def test_cli_version(capsys, monkeypatch):
    ...
    monkeypatch.setattr(sys, "argv", ["strepsuis-genphen", "--version"])
    with pytest.raises(SystemExit) as exc_info:
        main()
    assert exc_info.value.code == 0
    captured = capsys.readouterr()
...
def test_cli_missing_required_args(monkeypatch):
    ...
    monkeypatch.setattr(sys, "argv", ["strepsuis-genphen"])
    with pytest.raises(SystemExit) as exc_info:
        main()
    assert exc_info.value.code != 0
...
def test_cli_with_invalid_data_dir(monkeypatch):
    ...
    monkeypatch.setattr(
        sys,
        "argv",
        ["strepsuis-genphen", "--data-dir", "/nonexistent/path", "--output", "/tmp/output"],
    )
    result = main()
    assert result != 0
