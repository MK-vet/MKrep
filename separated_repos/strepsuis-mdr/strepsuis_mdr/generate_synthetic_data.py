...
def validate_synthetic_data(
    ...
    if actual_rows == expected_rows:
        results["checks"].append(f"✓ Row count: {actual_rows} (expected {expected_rows})")
    else:
    ...
    for col in metadata.antibiotic_columns:
        if col in data.columns:
            results["checks"].append(f"✓ Column exists: {col}")
        else:
    ...
    for col in metadata.antibiotic_columns + metadata.gene_columns:
        ...
        if unique_vals.issubset({0, 1}):
            results["checks"].append(f"✓ Binary data: {col}")
        else:
            results["warnings"].append(f"⚠ Non-binary values in {col}: {unique_vals}")
    ...
    for col, expected_prev in metadata.true_prevalences.items():
        ...
        if abs(actual_prev - expected_prev) / max(expected_prev, 0.01) < 0.3:
            results["checks"].append(
                f"✓ Prevalence {col}: {actual_prev:.2f} (expected ~{expected_prev:.2f})"
            )
        else:
            results["warnings"].append(
                f"⚠ Prevalence deviation {col}: {actual_prev:.2f} (expected ~{expected_prev:.2f})"
            )
    ...
    if len(metadata.true_mdr_status) == len(data):
        ...
        results["checks"].append(f"✓ MDR proportion: {mdr_prop:.2%} ({mdr_count} isolates)")
